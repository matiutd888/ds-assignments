    ...
    for path in paths {
        ...
        if let Some(filename) = path.unwrap().path().file_name() {
            if let Some(string) = filename.to_str() {
                let mut split = string.split(&SectorsManagerImpl::delimeter);
                ...
                if v.len() == 3 {
                    ...
                    if idx.is_ok() && timestamp.is_ok() && write_rank.is_ok() {
                        assert!(metadata_map.insert(idx.unwrap(), (timestamp.unwrap(), write_rank.unwrap())) == None);
                    }
                }
            }
        }
...
struct SectorsManagerImpl {
    sectors_metadata: HashMap<SectorIdx, Metadata>,
    root_storage_dir: PathBuf,
}

impl SectorsManagerImpl {
    const delimeter: String = String::from("-");
    const ZERO_SECTOR: Vec<u8> = vec![0; constants::SECTOR_SIZE_BYTES];
...
impl SectorsManager for SectorsManagerImpl {
    /// Returns 4096 bytes of sector data by index.
    async fn read_data(&self, idx: SectorIdx) -> SectorVec {
        let result_vec = if let Some(metadata) = self.sectors_metadata.get(&idx) {
    ...
    /// there.
    async fn read_metadata(&self, idx: SectorIdx) -> (u64, u8) {
        if let Some(metadata) = self.sectors_metadata.get(&idx) {
            metadata.clone()
        } else {
    ...
    async fn write(&self, idx: SectorIdx, sector: &(SectorVec, u64, u8)) {
        ...
        if let Some(metadata) = self.sectors_metadata.get(&idx) {
            let current_path = self.append_to_path(Self::create_filename(&idx, metadata));
