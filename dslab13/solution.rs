...
pub(crate) type QueryResultPollCallback = Box<dyn FnOnce(Option<u64>) -> Pin<Box<dyn Future<Output = ()> + Send>> + Send>;
...
impl Node {
    pub(crate) async fn new(
        ...
        let self_ref = system.register_module(Self {
            uuid,
            rs,
            pss,
...
impl Handler<QueryInstallMsg> for Node {
    async fn handle(&mut self, _self_ref: &ModuleRef<Self>, msg: QueryInstallMsg) {
        if msg.bits_per_instance == 0 || msg.bits_per_instance > u32::BITS as usize ||
                msg.bits_per_instance % 8 != 0 || msg.num_instances == 0 {
            return;
        }
        // TODO: implement
        unimplemented!()
